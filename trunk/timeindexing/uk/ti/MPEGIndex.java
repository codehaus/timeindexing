...
public class MPEGIndex {
	...
	if (args.length == 2) {
	    ...
	    new MPEGIndex(args[0], args[1]);
	} else if (args.length == 1) {
	    // have tifile name only, assume input is stdin
	    new MPEGIndex(args[0]);
	} else {
	    help(System.err);
	}
    }
    ...
    /**
     * Build a MPEGIndex object with a timeindex filename and an input filename.
     * If the input filename is "-", use stdin.
     */
    public MPEGIndex(String tiFileName, String inputFileName) {
    ...
    /**
     * Build a MPEGIndex object with a timeindex filename only
     */
    public MPEGIndex(String tiFileName) {
	InputStream input = null;
    ...
    /**
     * Build a MPEGIndex object with a timeindex file name and an input stream
     */
    public MPEGIndex(String tiFileName, InputStream input) {
	create(tiFileName, input);
    }
	...
	if (input == null) {
	    throw new NullPointerException("Input is null in MPEGIndex::create()");
	}
